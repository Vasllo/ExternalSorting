...
void cria_arvore_binaria_indice_primario(FILE *arqIndicePrimario, int nCli)
{
...
void criar_arvore_binaria_externa(FILE *arq_entrada, FILE *arq_saida)
{
    rewind(arq_entrada);
    Cliente cli;
    while (fread(&cli, sizeof(Cliente), 1, arq_entrada) == 1)
    {
        inserir_item_arvore_binaria_externa(cli, arq_saida);
    }
    inserir_item_arvore_binaria_externa(cli, arq_saida);
    ...
    if (elemento != -1)
    {
        ...
        printf("Cliente %s de codigo %d encontrado! \n", f->nome, f->codCliente);
    }
}
...
void buscar_recursivo_arvore_binaria_externa(long localraiz, int chave, Cliente **resposta, FILE *arq)
{
    ...
    if (fread(&aux, sizeof(TNoAX), 1, arq) == 1)
    {
        ...
        {
            *resposta = (Cliente *)malloc(sizeof(Cliente));
            **resposta = aux.item; //achou
        }
        else
        {
            if (aux.item.codCliente > chave)
