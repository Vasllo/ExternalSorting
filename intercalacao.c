...
void intercalacao_basico(char *nome_arquivo_saida, int num_p, Lista *nome_particoes)
{
    ...
    else
    {
        ...
        { //conseguiu abrir todos os arquivos
            ...
            for (int i = 0; i < num_p; i++)
            {
                if (v[i].cli->codCliente < menor)
                {
                    menor = v[i].cli->codCliente;
                    pos_menor = i;
                }
            }
            if (menor == INT_MAX)
            ...
            else
            {
                ...
                //atualiza posição pos_menor do vetor com proximo arquivo do arquivo
                v[pos_menor].aux_p++;
                fseek(v[pos_menor].f, v[pos_menor].aux_p * tamanho_registro(), SEEK_SET);
                Cliente *f = le_cliente(v[pos_menor].f);
                aux++;
                if (f == NULL)
                {
                    //arquivo estava vazio
                    //coloca HIGH VALUE nessa posiçao do vetor
                    v[pos_menor].func = cliente(INT_MAX, "", "", "", 0.0);
                }
                else
                {
                    v[pos_menor].func = f;
...
void intercalacao_arvore_de_vencedores(TPilha **pilha, int *vetTop, char *nome_arquivo_saida, int nParticoes, int nCli)
{
    FILE *out;
    ...
    Cliente **hash = malloc(nCli * sizeof(Cliente*));
    ...
    {
        Cliente *cli = pop(pilha[i], 0, &vetTop[i]);
        hash[cli->codCliente] = cli;
        TNoA *no = criaNo_arvore_binaria(cli->codCliente, -1);
        no->pilha = i;
        base[i] = no;
    }
    ...
    while (arvore->info != INT_MAX)
    {
        ...
        Cliente *cli = hash[infoVencedor];
        fseek(out, contArqSaida * tamanho_registro(), SEEK_SET);
        contArqSaida++;
        salva_cliente(cli, out);   //Salvando cliente no arquivo de saida
        int pilhaVencedor = vencedor->pilha;

        cli = pop(pilha[pilhaVencedor], 0, &vetTop[pilhaVencedor]);

        if (cli == NULL)
            arvore->info = INT_MAX;
        else
        {
            arvore->info = cli->codCliente;
            hash[cli->codCliente] = cli;
        }
